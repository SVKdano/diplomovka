...
def evaluate_baseline(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.Series, y_test: pd.Series):
    """
    Triviálna baseline: vždy predikuje najčastejšiu triedu.
    """
...
# --------------------------------------------------
# 10. Export zjednodušeného stromu cez Graphviz
# --------------------------------------------------
def export_simplified_tree_graphviz(model, feature_names, class_names, output_path="final_outputs/simplified_tree"):
    """
    Vygeneruje rozhodovací strom tak, že:
    - vnútorné uzly obsahujú len otázku
    - listy obsahujú len class
    Výstup uloží ako PDF aj SVG.
    """
    tree_ = model.tree_
...
def main():
    ...
    # Uloženie textového výpisu do súboru
    with open(f"final_outputs/tree_rules_{target_short_name}_ros.txt", "w", encoding="utf-8") as f:
        f.write(tree_rules)
    ...
    plt.tight_layout()
    plt.savefig(f"final_outputs/matrix_{target_short_name}_ros.png", bbox_inches="tight")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f"final_outputs/factors_{target_short_name}_ros.png", bbox_inches="tight")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f"final_outputs/tree_{target_short_name}_ros.png", bbox_inches="tight")
    plt.show()

    # Zjednodušený strom: len otázky a class v listoch
    export_simplified_tree_graphviz(
        model=tree_model,
        feature_names=list(X.columns),
        class_names=class_names,
        output_path=f"final_outputs/simplified_tree_{target_short_name}_ros_only_questions_and_class"
    )
