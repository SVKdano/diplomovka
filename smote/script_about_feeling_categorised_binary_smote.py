...
# --------------------------------------------------
# 3) Hľadanie najlepšieho modelu (dynamické CV + SMOTE)
# --------------------------------------------------
def find_best_model(X_train: pd.DataFrame, y_train: pd.Series) -> ImbPipeline:
    ...
    if min_class < 2:
        raise ValueError("SMOTE vyžaduje aspoň 2 vzorky v každej triede na trénovanie.")
    # pri SMOTE vnútri CV treba nechať v tréningovej časti foldov dosť minoritných vzoriek
    cv_folds = int(max(2, min(10, min_class - 1)))
    cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=42)
    min_class_in_inner_train = min_class - math.ceil(min_class / cv_folds)
    smote_k = max(1, min(5, min_class_in_inner_train - 1))
...
def main():
    ...
    plt.ylabel("Skutočné")
    plt.title("Matica zámien (Confusion Matrix) - kategórie zdravotného pocitu (SMOTE)")
    plt.tight_layout()
    plt.savefig("outputs/matrix_feeling_categorised_binary_smote.png", bbox_inches="tight")
    plt.show()
    ...
    plt.title("Top 10 faktorov ovplyvňujúcich kategóriu zdravotného pocitu (SMOTE)")
    plt.xlabel("Dôležitosť (Gini importance)")
    plt.tight_layout()
    plt.savefig("outputs/factors_feeling_categorised_binary_smote.png", bbox_inches="tight")
    plt.show()
    ...
    out_name = "outputs/final_tree_feeling_categorised_binary_smote.png"
