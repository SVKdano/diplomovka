...
# --------------------------------------------------
# 6. Hľadanie najlepšieho klasifikátora
# --------------------------------------------------
def find_best_model(X_train: pd.DataFrame, y_train: pd.Series):
    ...
    SMOTE je správne aplikované iba vo vnútri CV foldov cez pipeline.
    """
...
# --------------------------------------------------
# 8. Export zjednodušeného stromu cez Graphviz
# --------------------------------------------------
...
def main():
    ...
    plt.tight_layout()
    plt.savefig("final_outputs/matrix_scoring.png", bbox_inches="tight")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("final_outputs/factors_scoring.png", bbox_inches="tight")
    plt.show()
    ...
    with open("final_outputs/tree_rules.txt", "w", encoding="utf-8") as f:
        f.write(tree_rules)
    ...
    plt.tight_layout()
    plt.savefig("final_outputs/final_tree_classification_scoring.png", bbox_inches="tight")
    plt.show()
    ...
    export_simplified_tree_graphviz(
        ...
        feature_names=list(X.columns),
        class_names=class_names,
        output_path="final_outputs/simplified_tree_only_questions_and_class"
    )
    ...
    print("\nTextový výpis stromu bol uložený do: outputs/tree_rules.txt")
